...
C     path:      /home/kato/code/rrtm/SCCS/s.setcoef.f
C     revision:  2.11
C     created:   04/16/98  16:37:42
...
c      COMMON /SURFACE/  TBOUND,IREFLECT,SEMISS(NBANDS)
c      COMMON /SPECIES/  COLDRY(MXLAY),WKL(35,MXLAY),WBROAD(MXLAY),
c     &                  COLMOL(MXLAY),NMOL

C  Output
c      COMMON /PROFDATA/ LAYTROP,LAYSWTCH,LAYLOW,COLH2O(MXLAY),
c     &                  COLCO2(MXLAY),COLO3(MXLAY),COLN2O(MXLAY),
c     &                  COLCH4(MXLAY),COLO2(MXLAY),CO2MULT(MXLAY)
c      COMMON /INTFAC/   FAC00(MXLAY),FAC01(MXLAY),
c     &                  FAC10(MXLAY),FAC11(MXLAY)
c      COMMON /INTIND/   JP(MXLAY),JT(MXLAY),JT1(MXLAY)
c      COMMON /SELF/     SELFFAC, SELFFRAC, INDSELF
c      COMMON /FOREIGN/  FORFAC(MXLAY)
c      COMMON /PLNKDAT/  PLANKLAY(MXLAY,NBANDS),
...
C        surface, level, and layer temperatures.
         ...
         ELSEIF (INDLAY .GT. 180) THEN
            INDLAY = 180
         ENDIF
         TLAYFRAC = TAVEL - 159. - FLOAT(INDLAY)
         INDLEV = TZ - 159.
         IF (INDLEV .LT. 1) THEN
            INDLEV = 1
...
c
c        For this version, the code usually compute the broadband
c        irradiance. So istart is not equal 16.
     ...
     &              T0FRAC * DBDTLEV
              ...
              PLANKLAY = TOTPLK16(INDLAY) + 
     &             TLAYFRAC * DBDTLAY
              PLANKLEV = TOTPLK16(INDLEV) + 
     &             TLEVFRAC * DBDTLEV
            end if
         ELSE
            IF (LAY.EQ.1) THEN
...
C        two values that the layer pressure lies.
         PLOG = DLOG(PAVEL)
         JP = INT(36. - 5*(PLOG+0.04))
         IF (JP .LT. 1) THEN
            JP = 1
         ELSEIF (JP .GT. 58) THEN
            JP = 58
         ENDIF
         JP1 = JP + 1
...
C        layer temperature falls.
         JT = INT(3. + (TAVEL-TREF(JP))/15.)
         IF (JT .LT. 1) THEN
            JT = 1
         ELSEIF (JT .GT. 4) THEN
            JT = 4
         ENDIF
         FT = ((TAVEL-TREF(JP))/15.) - FLOAT(JT-3)
         JT1 = INT(3. + (TAVEL-TREF(JP1))/15.)
         IF (JT1 .LT. 1) THEN
            JT1 = 1
         ELSEIF (JT1 .GT. 4) THEN
            JT1 = 4
         ENDIF
...
c SK, these parameters are assigned in atmosphere_layers_tau_mlawer*******
c         LAYTROP =  LAYTROP + 1
C        For one band, the "switch" occurs at ~300 mb. 
c         IF (PLOG .GE. 5.76) LAYSWTCH = LAYSWTCH + 1
c         IF (PLOG .GE. 6.62) LAYLOW = LAYLOW + 1
c SK *********************************************************************
C
         FORFAC = SCALEFAC / (1.+WATER)
...
C        self-continuum in the calculation of absorption coefficient.
         ...
         SELFFRAC = FACTOR - FLOAT(INDSELF + 7)
...
C        Calculate needed column amounts.
         ...
         COLCH4 = 1.E-20 * WKL_6
         COLO2 = 1.E-20 * WKL_7
         IF (COLCO2 .EQ. 0.) COLCO2 = 1.E-32 * COLDRY
         IF (COLN2O .EQ. 0.) COLN2O = 1.E-32 * COLDRY
         IF (COLCH4 .EQ. 0.) COLCH4 = 1.E-32 * COLDRY
C        Using E = 1334.2 cm-1.
         CO2REG = 3.55E-24 * COLDRY
         CO2MULT= (COLCO2 - CO2REG) *
     &        272.63*EXP(-1919.4/TAVEL)/(8.7604E-4*TAVEL)
         GO TO 5400
