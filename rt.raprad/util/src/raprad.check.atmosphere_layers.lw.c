...
void check_atmosphere_layers_lw(int i, int j, SpectralModel *sm, Rt *rt, Sun *suna, Brdf *d)
{
  int k;
  FILE *fpta, *fopen();
  ...
  for (k=1; k<=rt->layers_number; ++k) {
    ...
    /*------------------------------------------------------------------------*/
    /* Print the  Planck function.                                            */
    /*------------------------------------------------------------------------*/

    fprintf(fpta, " %15.5e", rt->planklayd[k]);

    /*------------------------------------------------------------------------*/
    /* Print the  weight.                                                     */
    /*------------------------------------------------------------------------*/

    fprintf(fpta, " %15.5e",  sm->alphad[i][j][k]);
