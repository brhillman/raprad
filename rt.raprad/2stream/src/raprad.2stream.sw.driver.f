     ...
     &   solconst,nlayers,amu0,
     &   w0_temp,g0_temp,suralb,
...
!     local namespace
      real*8 fup(ilayer), fdn(ilayer), fdi(ilayer)
      real*8 fnet(ilayer)
      ...
      real*8 den, denom, dist_cor, fo, g0t
...
c making the delta approximation
 ...
 20   continue
       ...
       if (amu0 .gt. 0.0) then
         call twostrsw(
     &     ntaujs,taul,w0,g0,suralb,
     &     b1,b2,el1,el2,em1,em2,af,bf,ef, 
     &     ak)

         call addsw(
     &     ntaujs,taul,w0,g0,suralb,opd,opdnd,ak,
     &     b1,b2,b3,em1,em2,el1,el2,af,bf,ef,
     &     amu0,fnet,fup,fdn,fdi)
       end if
...
c     BRH these are not used
c200  format('layer z(km)  prs(mb)   sw down    sw up')
c220  format(i4,2x,f6.2,2x,f7.1,2x,f9.3,3x,f9.3)
c240  format('mu_0=',f6.4,5x,'radau=',f6.4)
